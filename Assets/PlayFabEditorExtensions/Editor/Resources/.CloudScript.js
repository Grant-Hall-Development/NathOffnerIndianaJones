handlers.SetHighscore = function(args)
{
    
    var score = args.score;
    log.debug("you passed in" + score);
    ...
    var highScore = parseInt(result.Data["Highscore"].Value);

    if(score > highScore)
    {
        var request = server.UpdateUserReadOnlyData({
            PlayFabId: currentPlayerId,
            Data: 
            {
                Highscore: score
...
handlers.AddBananaTimeValues = function()
{
    var playerData = {
        CurrentPercentage: 1,
        CurrentTimeStamp: new Date().getTime(),
        NumberOfBananasCollectedThreshold: 0
    };
    
      var request = server.UpdateUserData({
        PlayFabId: currentPlayerId,
        Data: playerData
    });
}

handlers.UpdateHasCompletedTutorial = function(args)
{
    var request = server.UpdateUserReadOnlyData({
        PlayFabId: currentPlayerId,
        Data: {
...
handlers.SetIsFirstTime = function(args)
{
    var request = server.UpdateUserReadOnlyData({
        PlayFabId: currentPlayerId,
        Data: {
...
handlers.AdjustBananas = function(args)
{
...
handlers.SetServerTimeForPlayer = function()
{
    var request = server.UpdateUserData({
        PlayFabId: currentPlayerId,
        Data: {
...
handlers.createNewTitlePlayer = function (args, context)
{
    ...
    var request2 = server.UpdateUserData({
        PlayFabId: currentPlayerId,
        Data: playerData
    });
...
handlers.getBananas = function (args, context)
{
    var playerInventory = server.GetUserInventory({
        PlayFabId: currentPlayerId,
    })
    var currency = playerInventory.VirtualCurrency["BN"];
